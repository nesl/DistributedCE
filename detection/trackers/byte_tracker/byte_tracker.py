...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, detected_class, detected_extra):
    ...
    def activate(self, kalman_filter, frame_id):
        ...
        # print(self.tlbr)
        activated = False
        if self.allowed_track_entrance(self.tlbr):
...
class BYTETracker(object):
    def __init__(self, args, match_class_filter, recover_lost_track, buffer_zone, ignore_stationary,\
         frame_rate):
    ...
    def update(self, output_results, img_info, img_size, detected_class, image, detected_extra):
        ...
        if len(dets) > 0:
            '''Detections'''
            detections = [STrack(STrack.tlbr_to_tlwh(tlbr), s, detected_class_keep[z_idx], detected_extra_keep[z_idx]) for
